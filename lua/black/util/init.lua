local highlights = require "black.highlights"
...
local prepare_environment = function()
    ...
    if not settings.disable.colored_cursor then
        ...
        local exit_group  = vim.api.nvim_create_augroup("BlackExit", { clear = true })
...
local apply_contrast = function(contrast_settings)
    ...
    -- clean up autogroups if the theme is not black
    vim.api.nvim_create_autocmd("ColorScheme", { callback = function()
        if vim.g.colors_name ~= "black" then
            vim.api.nvim_del_augroup_by_name("Black")
        end
    end, group = group })
